...
dcl-proc ensureTables;
  ...
  runDdl('CREATE TABLE QTEMP.MOCK_STUB (STUBID INT NOT NULL, '
       ...
       + 'RTNCNT INT NOT NULL)');
  runDdl('CREATE TABLE QTEMP.MOCK_SARG (STUBID INT NOT NULL, '
       + 'PARMNO SMALLINT NOT NULL, MATCHER CHAR(10) NOT NULL, '
       + 'VAL VARCHAR(1024) NOT NULL)');
  runDdl('CREATE TABLE QTEMP.MOCK_SRTN (STUBID INT NOT NULL, '
       + 'SEQ INT NOT NULL, VAL VARCHAR(1024) NOT NULL)');
  runDdl('CREATE TABLE QTEMP.MOCK_SSET (STUBID INT NOT NULL, '
       + 'PARMNO SMALLINT NOT NULL, VAL VARCHAR(1024) NOT NULL)');
  runDdl('CREATE TABLE QTEMP.MOCK_CALL (CALLID INT NOT NULL, '
       + 'OBJ CHAR(10) NOT NULL, PROC VARCHAR(4096) NOT NULL, '
       + 'PARMCNT INT NOT NULL, STUBID INT NOT NULL, '
       + 'VERIFIED CHAR(1) NOT NULL, TS TIMESTAMP NOT NULL)');
  runDdl('CREATE TABLE QTEMP.MOCK_CARG (CALLID INT NOT NULL, '
...
dcl-proc dropTables;
  runDdl('DROP TABLE QTEMP.MOCK_OBJ');
  runDdl('DROP TABLE QTEMP.MOCK_PROC');
  runDdl('DROP TABLE QTEMP.MOCK_SIG');
  runDdl('DROP TABLE QTEMP.MOCK_STUB');
  runDdl('DROP TABLE QTEMP.MOCK_SARG');
  runDdl('DROP TABLE QTEMP.MOCK_SRTN');
  runDdl('DROP TABLE QTEMP.MOCK_SSET');
  runDdl('DROP TABLE QTEMP.MOCK_CALL');
  runDdl('DROP TABLE QTEMP.MOCK_CARG');
  runDdl('DROP ALIAS QTEMP.MOCK_SRCW');
  runDdl('DROP ALIAS QTEMP.MOCK_BNDR');
  gTablesOk = *off;
end-proc;
...
dcl-proc deleteStubs;
  ...
  exec sql delete from qtemp.mock_sarg where stubid in
    (select stubid from qtemp.mock_stub where obj = :obj or :obj = '*ALL');
  exec sql delete from qtemp.mock_srtn where stubid in
    (select stubid from qtemp.mock_stub where obj = :obj or :obj = '*ALL');
  exec sql delete from qtemp.mock_sset where stubid in
    (select stubid from qtemp.mock_stub where obj = :obj or :obj = '*ALL');
  exec sql delete from qtemp.mock_stub where obj = :obj or :obj = '*ALL';
end-proc;
...
dcl-proc deleteCalls;
  ...
  exec sql delete from qtemp.mock_carg where callid in
    (select callid from qtemp.mock_call where obj = :obj or :obj = '*ALL');
  exec sql delete from qtemp.mock_call where obj = :obj or :obj = '*ALL';
end-proc;
...
dcl-proc forgetObj;
  ...
  deleteStubs(obj);
  deleteCalls(obj);
  exec sql delete from qtemp.mock_sig where obj = :obj or :obj = '*ALL';
  exec sql delete from qtemp.mock_proc where obj = :obj or :obj = '*ALL';
  exec sql delete from qtemp.mock_obj where obj = :obj or :obj = '*ALL';
end-proc;
...
dcl-proc objInfo;
  ...
  exec sql select objtype, behavior, built, reallib
             ...
             from qtemp.mock_obj where obj = :obj
...
dcl-proc resolveProc;
  ...
  if objType = '*PGM';
    if p <> '' and p <> '*PGM';
      setErr(err : 'MCK0012' : 'Mock ' + %trim(obj) + ' is a program; '
  ...
  if p = '' or p = '*PGM';
    setErr(err : 'MCK0012' : 'PROC is required for service program mock '
         + %trim(obj));
    return *off;
  endif;

  exec sql select proc, kind, declared into :procOut, :kind, :declared
             from qtemp.mock_proc where obj = :obj and proc = :p
             fetch first 1 row only;
  if sqlcode = 0;
    return *on;
  endif;

  exec sql select count(*) into :n from qtemp.mock_proc
             where obj = :obj and upper(proc) = upper(:p);
  if n = 1;
    exec sql select proc, kind, declared into :procOut, :kind, :declared
               from qtemp.mock_proc
               where obj = :obj and upper(proc) = upper(:p)
               fetch first 1 row only;
    return *on;
  endif;

  setErr(err : 'MCK0012' : %trim(obj) + ' does not export ' + p);
  return *off;
end-proc;
...
dcl-proc loadSig;
  dcl-pi *n;
    obj char(10) const;
    proc varchar(4096) const;
    defs likeds(mock_def_t) dim(64);
    nDefs int(10);
    rtnDef likeds(mock_def_t);
    hasRtn ind;
  end-pi;
  dcl-s parmNo int(5);
  dcl-s type char(10);
...
dcl-proc insertSig;
  ...
  dec = def.dec;
  passing = def.passing;
  exec sql insert into qtemp.mock_sig
...
dcl-proc readMatchers;
  dcl-pi *n ind;
    ...
    nM int(10);
    defs likeds(mock_def_t) dim(64) const;
    nDefs int(10) const;
  ...
  nM = lstCount(blob);
  if nM > MOCK_MAXP;
    setErr(err : 'MCK0014' : 'At most 64 argument matchers are allowed');
    return *off;
  endif;
  for i = 1 to nM;
    e = lstEntry(blob : i);
    ...
    if m(i).parmNo < 1 or m(i).parmNo > MOCK_MAXP;
      setErr(err : 'MCK0014' : 'ARGS entry ' + %char(i)
           + ': parameter number must be 1 to 64');
      return *off;
    endif;
    if not mock_validMatcher(m(i).matcher);
      setErr(err : 'MCK0014' : 'ARGS entry ' + %char(i) + ': '
...
dcl-proc loadCallArgs;
  ...
  exec sql declare cCarg cursor for
    select parmno, state, val from qtemp.mock_carg
  ...
  dow sqlcode = 0;
    ...
    if parmNo >= 1 and parmNo <= MOCK_MAXP;
      st(parmNo) = state;
      vals(parmNo) = v;
      if parmNo > nArgs;
        nArgs = parmNo;
...
dcl-proc callMatches;
  ...
  for i = 1 to nM;
    ...
    if not mock_match(m(i).matcher : m(i).val : st(m(i).parmNo)
                      : vals(m(i).parmNo) : d);
      return *off;
    endif;
  endfor;
...
dcl-proc loadCallIds;
  ...
  exec sql declare cCall cursor for
    select callid from qtemp.mock_call
...
// MOCKPGM -----------------------------------------------------------
dcl-proc mock_cl_defPgm export;
  dcl-pi *n;
    obj char(10) const;
  ...
  n = lstCount(p);
  if n > MOCK_MAXP;
    setErr(err : 'MCK0014' : 'At most 64 parameters can be declared');
    return;
  endif;
  for i = 1 to n;
    readDef(lstEntry(p : i) : defs(i) : *off);
    if not mock_normDef(defs(i) : m256);
      setErr(err : 'MCK0014' : 'PARMS entry ' + %char(i) + ': ' + m256);
      return;
    endif;
  endfor;

  forgetObj(obj);
  exec sql insert into qtemp.mock_obj
    values(:obj, '*PGM', :behavior, :realLib, 'N');
  if sqlcode < 0;
    setErr(err : 'MCK0015' : sqlFailText('Register mock'));
    return;
  endif;
  exec sql insert into qtemp.mock_proc
    values(:obj, '*PGM', 1, 'PROC', 0, 'Y');
  for i = 1 to n;
    insertSig(obj : '*PGM' : i : defs(i));
  endfor;

  if not mock_genPgm(obj : n : msg);
    setErr(err : 'MCK0015' : msg);
    return;
  endif;
...
// MOCKSRVPGM --------------------------------------------------------
dcl-proc mock_cl_defSrv export;
  dcl-pi *n;
    obj char(10) const;
    behavior char(7) const;
    realLib char(10) const;
  ...
  clearErr(err);
  ensureTables();
  if not mock_readExports(obj : names : n : msg);
    setErr(err : 'MCK0013' : msg);
    return;
  endif;
  if n = 0;
    setErr(err : 'MCK0013' : 'No exports found in the binder source for '
         + %trim(obj));
    return;
  endif;
  ...
  forgetObj(obj);
  exec sql insert into qtemp.mock_obj
    values(:obj, '*SRVPGM', :behavior, :realLib, 'N');
  if sqlcode < 0;
    setErr(err : 'MCK0015' : sqlFailText('Register mock'));
    return;
  endif;
...
// MOCKPROC ----------------------------------------------------------
dcl-proc mock_cl_defProc export;
  dcl-pi *n;
    obj char(10) const;
    procVary char(258);
  ...
  if not objInfo(obj : objType : behavior : built : realLib);
    setErr(err : 'MCK0011' : 'Mock ' + %trim(obj) + ' does not exist. '
         + 'Create it with MOCKSRVPGM first');
    return;
  endif;
  if objType <> '*SRVPGM';
    setErr(err : 'MCK0012' : 'MOCKPROC applies to service program mocks; '
         + %trim(obj) + ' is a program mock (use MOCKPGM PARMS)');
    return;
  endif;
  ...
  if kind = 'DATA';
    setErr(err : 'MCK0012' : proc + ' is a data export, not a procedure');
    return;
  endif;
  ...
  if lstCount(p) > 0;
    ...
    if rtnDef.type <> '*NONE' and rtnDef.type <> ' ';
      if not mock_normDef(rtnDef : m256);
        setErr(err : 'MCK0014' : 'RTNTYPE: ' + m256);
        return;
      endif;
      hasRtn = *on;
    endif;
  ...
  exec sql delete from qtemp.mock_sig where obj = :obj and proc = :proc;
  if hasRtn;
  ...
  exec sql update qtemp.mock_proc set declared = 'Y'
            where obj = :obj and proc = :proc;
  exec sql update qtemp.mock_obj set built = 'N' where obj = :obj;
  err.text = proc;
end-proc;

// MOCKBUILD ---------------------------------------------------------
dcl-proc mock_cl_genSrv export;
  dcl-pi *n;
    obj char(10) const;
    err likeds(mock_err_t);
  end-pi;
  ...
  if objType <> '*SRVPGM';
    setErr(err : 'MCK0012' : 'MOCKBUILD applies to service program mocks; '
         + %trim(obj) + ' is a program mock');
    return;
  endif;
  if not mock_genSrv(obj : msg);
    setErr(err : 'MCK0015' : msg);
  endif;
end-proc;

dcl-proc mock_cl_setBuilt export;
  dcl-pi *n;
    obj char(10) const;
    err likeds(mock_err_t);
  end-pi;
  clearErr(err);
  ensureTables();
  exec sql update qtemp.mock_obj set built = 'Y' where obj = :obj;
end-proc;

// Record a failure detected by a command processing program
dcl-proc mock_cl_fail export;
  dcl-pi *n;
    msgId char(7) const;
    text char(512) const;
    err likeds(mock_err_t);
...
dcl-proc mock_cl_when export;
  ...
  if not objInfo(obj : objType : behavior : built : realLib);
    setErr(err : 'MCK0011' : 'Mock ' + %trim(obj) + ' does not exist. '
         + 'Create it with MOCKPGM or MOCKSRVPGM first');
    return;
  endif;
  if not resolveProc(obj : objType : varyText(%addr(procVary) : 256)
                     : proc : kind : declared : err);
  ...
  lbl = label(obj : proc);
  if kind = 'DATA';
    setErr(err : 'MCK0012' : proc + ' is a data export and cannot be '
         + 'stubbed');
    return;
  endif;
  ...
  nRtn = lstCount(p);
  if nRtn > 32;
    setErr(err : 'MCK0014' : 'At most 32 RETURN values are allowed');
    return;
  endif;
  for i = 1 to nRtn;
  ...
  if nRtn > 0 and not hasRtn;
    setErr(err : 'MCK0014' : lbl + ' has no declared return value. '
         + 'Declare RTNTYPE with MOCKPROC');
    return;
  endif;
  for i = 1 to nRtn;
    if not canEncode(rtnDef : rtnVal(i) : m256);
      setErr(err : 'MCK0014' : 'RETURN value ' + %char(i) + ': ' + m256);
      return;
    endif;
  endfor;
  ...
  nSet = lstCount(p);
  if nSet > MOCK_MAXP;
    setErr(err : 'MCK0014' : 'At most 64 SETPARM entries are allowed');
    return;
  endif;
  for i = 1 to nSet;
    e = lstEntry(p : i);
    ...
    if setNo(i) < 1 or setNo(i) > nDefs;
      setErr(err : 'MCK0014' : 'SETPARM entry ' + %char(i) + ': parameter '
    ...
    if not canEncode(defs(setNo(i)) : setVal(i) : m256);
      setErr(err : 'MCK0014' : 'SETPARM entry ' + %char(i) + ': ' + m256);
      return;
    endif;
  endfor;
  ...
  if times < -1;
    setErr(err : 'MCK0014' : 'TIMES must be *ALWAYS or a positive number');
    return;
  endif;

  exec sql select coalesce(max(stubid), 0) + 1 into :id
             from qtemp.mock_stub;
  exec sql insert into qtemp.mock_stub
    values(:id, :obj, :proc, :times, 0, :thrId, :thrMsgf, :thrLib,
           :thrDta, :nRtn);
  if sqlcode < 0;
    setErr(err : 'MCK0015' : sqlFailText('Save stub'));
    return;
  endif;
  for i = 1 to nM;
    parmNo = m(i).parmNo;
    mt = m(i).matcher;
    v = m(i).val;
    exec sql insert into qtemp.mock_sarg values(:id, :parmNo, :mt, :v);
  endfor;
  for i = 1 to nRtn;
    seq = i;
    v = rtnVal(i);
    exec sql insert into qtemp.mock_srtn values(:id, :seq, :v);
  endfor;
  for i = 1 to nSet;
    parmNo = setNo(i);
    v = setVal(i);
    exec sql insert into qtemp.mock_sset values(:id, :parmNo, :v);
  endfor;
end-proc;

// MOCKVERIFY --------------------------------------------------------
dcl-proc mock_cl_verify export;
  dcl-pi *n;
    obj char(10) const;
    procVary char(258);
    args char(1) options(*varsize);
  ...
  if want < 0;
    setErr(err : 'MCK0014' : 'TIMES count cannot be negative');
    return;
  endif;
  ...
  if ok;
    for i = 1 to nIds;
      if hit(i);
        id = ids(i);
        exec sql update qtemp.mock_call set verified = 'Y'
                  where callid = :id;
      endif;
    endfor;
    return;
  ...
  setErr(err : 'MCK0200' : txt);
end-proc;

// MOCKNOMORE --------------------------------------------------------
dcl-proc mock_cl_noMore export;
  dcl-pi *n;
    obj char(10) const;
    err likeds(mock_err_t);
  end-pi;
  ...
  exec sql declare cUnv cursor for
    select callid, obj, proc from qtemp.mock_call
...
dcl-proc getArg;
  ...
  if callNo = -1;
    exec sql select coalesce(max(callid), 0) into :id
               from qtemp.mock_call where obj = :obj and proc = :proc;
  else;
    offs = callNo - 1;
    if offs < 0;
      offs = 0;
    endif;
    exec sql select callid into :id from qtemp.mock_call
              where obj = :obj and proc = :proc order by callid
  ...
  if id = 0;
    setErr(err : 'MCK0202' : label(obj : proc) + ' has no call number '
  ...
  p5 = parmNo;
  exec sql select state, val into :state, :val from qtemp.mock_carg
...
dcl-proc countCalls;
  ...
  dcl-s n int(10);
  exec sql select count(*) into :n from qtemp.mock_call
            where obj = :obj and proc = :proc;
  return n;
end-proc;

dcl-proc mock_cl_getArg export;
  dcl-pi *n;
    obj char(10) const;
    procVary char(258);
    callNo int(10) const;
...
dcl-proc mock_cl_list export;
  ...
  exec sql declare cObj cursor for
    select obj, objtype from qtemp.mock_obj
     where obj = :obj or :obj = '*ALL' order by obj;
  exec sql open cObj;
  dow sqlcode = 0 and count < 200;
    exec sql fetch next from cObj into :o, :t;
    if sqlcode <> 0;
      leave;
    endif;
    %subst(list : count * 17 + 1 : 17) = o + t;
    count += 1;
  enddo;
  exec sql close cObj;
end-proc;

dcl-proc mock_cl_forget export;
  dcl-pi *n;
    obj char(10) const;
    err likeds(mock_err_t);
  end-pi;
...
dcl-proc mock_cl_check export;
  ...
  exec sql declare cChk cursor for
    select obj, objtype, built from qtemp.mock_obj order by obj;
  exec sql open cChk;
  dow sqlcode = 0;
    exec sql fetch next from cChk into :o, :t, :built;
    if sqlcode <> 0;
      leave;
    endif;
    ...
    if hit = ' ';
      addLine(outLines : count : 'Mock ' + %trim(o) + ' ' + %trim(t)
            + ' was not found in the library list (run MOCKBUILD or '
            + 're-create the mock)');
    elseif hit <> 'QTEMP';
      addLine(outLines : count : 'Mock ' + %trim(o) + ' is hidden by '
            + %trim(hit) + '/' + %trim(o) + ', which is ahead of QTEMP '
            + 'in the library list');
    endif;
    if t = '*SRVPGM' and built <> 'Y';
      addLine(outLines : count : 'Service program mock ' + %trim(o)
            + ' has changes that are not built (run MOCKBUILD)');
    endif;
  enddo;
  exec sql close cChk;
  ...
  exec sql declare cBnd cursor for
    select b.bound_service_program_library, b.bound_service_program
      ...
      join qtemp.mock_obj m on m.obj = b.bound_service_program
...
// ==================================================================
// mock_invoke - runtime entry used by every generated stub
//   returns 1 when the stub must send thr as an escape message
// ==================================================================
dcl-proc mock_invoke export;
  dcl-pi *n int(10);
    obj char(10) const;
    proc varchar(4096) const;
  ...
  monitor;
    ...
    for i = 1 to nCap;
      ...
      elseif i <= nDefs;
        st(i) = 'P';
        vals(i) = mock_decode(ptrs(i) : defs(i));
      else;
        st(i) = 'P';
        vals(i) = '*UNDECLARED';
      endif;
    ...
    if chosen = 0;
      if behavior = '*STRICT';
        ...
        thr.msgDta = 'Unexpected call to ' + label(obj : proc) + args
                   + ' (strict mock has no matching MOCKWHEN)';
        gLastErr = thr.msgDta;
        return 1;
      endif;
      return 0;
    ...
    exec sql declare cSet cursor for
      select parmno, val from qtemp.mock_sset
    ...
    dow sqlcode = 0;
      ...
      if parmNo >= 1 and parmNo <= nDefs and parmNo <= nPassed;
        if st(parmNo) = 'P';
          mock_encode(ptrs(parmNo) : defs(parmNo) : v : m256);
        endif;
      endif;
    enddo;
    exec sql close cSet;
    ...
    if rtnCnt > 0 and hasRtn and rtnPtr <> *null;
      ...
      exec sql select val into :v from qtemp.mock_srtn
                where stubid = :sid and seq = :seq;
      if sqlcode = 0;
        mock_encode(rtnPtr : rtnDef : v : m256);
      endif;
    endif;
  on-error;
    return 0;
...
dcl-proc stubMatches;
  ...
  exec sql declare cSarg cursor for
    select parmno, matcher, val from qtemp.mock_sarg
  ...
  dow sqlcode = 0;
    ...
    if not mock_match(matcher : v : st(parmNo) : vals(parmNo) : d);
      ok = *off;
      leave;
    endif;
  enddo;
...
dcl-proc mock_run export;
  ...
  ec.bytesProv = 0;
  dta = gLastErr;
  qmhsndpm('MCK0300' : 'MOCKMSGF  ' + psds.lib : dta : %len(gLastErr)
          : '*ESCAPE' : '*' : 1 : key : ec);
end-proc;

dcl-proc mock_ok export;
  dcl-pi *n ind;
    cmd varchar(3000) const;
  end-pi;
  return runCmd(cmd);
...
dcl-proc mock_lastError export;
  dcl-pi *n varchar(512) end-pi;
  return gLastErr;
end-proc;

dcl-proc mock_arg export;
  dcl-pi *n varchar(1024);
    obj char(10) const;
    proc varchar(4096) const;
    callNo int(10) const;
