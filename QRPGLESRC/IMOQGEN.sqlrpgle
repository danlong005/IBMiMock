**free
// ------------------------------------------------------------------
// IMOQGEN - iMoq source generation
//   * stub source for *PGM and *SRVPGM mocks -> QTEMP/IMOQSRC(obj)
//   * export list from binder source         <- QTEMP/IMOQBND(obj)
...
dcl-proc srcWrite;
  ...
  exec sql drop alias qtemp.imoq_srcw;
  stmt = 'CREATE ALIAS QTEMP.IMOQ_SRCW FOR QTEMP.' + %trim(file)
       + ' (' + %trim(mbr) + ')';
  ...
  exec sql delete from qtemp.imoq_srcw;
  n = gLines;
  exec sql insert into qtemp.imoq_srcw (srcseq, srcdat, srcdta)
    :n rows values(:gSrc);
  if sqlcode < 0;
    code = sqlcode;
    exec sql get diagnostics condition 1 :t = message_text;
...
dcl-proc genCommon;
  srcLine('dcl-ds imoq_throw_t qualified template;');
  srcLine('  msgId char(7);');
  srcLine('  msgf char(10);');
  srcLine('  msgfLib char(10);');
  srcLine('  msgDta varchar(512);');
  srcLine('end-ds;');
  srcLine('dcl-pr imoq_invoke int(10) extproc(''IMOQ_INVOKE'');');
  ...
  srcLine('  thr likeds(imoq_throw_t);');
  srcLine('end-pr;');
  srcLine('dcl-pr imoq_sndEsc extpgm(''QMHSNDPM'');');
...
dcl-proc genBody;
  ...
  srcLine('  dcl-ds mockThr likeds(imoq_throw_t);');
  ...
  srcLine('  if imoq_invoke(' + objLit + ' :');
  srcLine('       ' + procLit + ' :');
  srcLine('       mockN : %addr(mockPtrs) : ' + rtnExpr
        + ' : mockThr) = 1;');
  srcLine('    imoq_sndEsc(mockThr.msgId : mockThr.msgf + mockThr.msgfLib');
...
// ==================================================================
// imoq_genPgm - program stub (64 generic by-reference parameters)
// ==================================================================
dcl-proc imoq_genPgm export;
  dcl-pi *n ind;
    obj char(10) const;
  ...
  srcBegin();
  srcLine('**free');
  srcLine('// iMoq generated stub for *PGM ' + %trim(obj)
        + ' - do not edit');
  srcLine('ctl-opt option(*srcstmt:*nodebugio);');
  genCommon();
  srcLine('dcl-pi *n;');
  for i = 1 to IMOQ_MAXP;
  ...
  srcLine('if imoq_invoke(' + quote(%trim(obj)) + ' : ''*PGM'' :');
  ...
  return srcWrite('IMOQSRC' : obj : msg);
end-proc;

// ==================================================================
// imoq_genSrv - service program stub module
// ==================================================================
dcl-proc imoq_genSrv export;
  dcl-pi *n ind;
    obj char(10) const;
    msg varchar(512);
  end-pi;
  ...
  exec sql declare cGenProc cursor for
    select proc, kind, datasize from qtemp.imoq_proc
  ...
  exec sql select expmode, signature into :mode, :sig
             from qtemp.imoq_obj where obj = :obj;

  if nProcs = 0;
    msg = 'Mock ' + %trim(obj) + ' has no exports';
    if mode = 'PROC';
      msg = 'Mock ' + %trim(obj) + ' has no procedures yet. Declare each '
          + 'one with IMOQPROC before IMOQBUILD';
    endif;
    return *off;
  endif;
  ...
  srcBegin();
  srcLine('**free');
  srcLine('// iMoq generated stub for *SRVPGM ' + %trim(obj)
        + ' - do not edit');
  ...
  for i = 1 to nProcs;
    if kinds(i) = 'DATA';
      if %len(procs(i)) > 180;
        msg = 'Export name too long for iMoq: ' + %subst(procs(i):1:60);
        return *off;
      endif;
  ...
  for i = 1 to nProcs;
    ...
    srcLine('  end-pi;');
    if hasRtn;
      srcLine('  dcl-s mockRtn ' + imoq_rpgType(rtnDef) + ' inz;');
  ...
  if not srcWrite('IMOQSRC' : obj : msg);
    return *off;
  endif;

  // SRCFILE(*NONE): generate the binder source from the declared procs
  if mode = 'PROC';
    srcBegin();
    if sig = '' or sig = '*GEN';
      srcLine('STRPGMEXP PGMLVL(*CURRENT) SIGNATURE(*GEN)');
    else;
      srcLine('STRPGMEXP PGMLVL(*CURRENT) SIGNATURE(''' + sig + ''')');
    endif;
    for i = 1 to nProcs;
      srcLine('  EXPORT SYMBOL("' + procs(i) + '")');
    endfor;
    srcLine('ENDPGMEXP');
    return srcWrite('IMOQBND' : obj : msg);
  endif;
  return *on;
end-proc;
...
dcl-proc parmDecl;
  ...
  when def.passing = '*VALUE';
    return imoq_rpgType(def) + ' value options(*nopass)';
  endsl;
  // *CONST is passed by reference like *REF; declaring it without
  // CONST lets the stub take the parameter's address
  return imoq_rpgType(def) + ' options(*nopass:*omit)';
end-proc;

// ==================================================================
// imoq_readExports - export symbols of the *CURRENT export block
// ==================================================================
dcl-proc imoq_readExports export;
  dcl-pi *n ind;
    obj char(10) const;
  ...
  exec sql drop alias qtemp.imoq_bndr;
  stmt = 'CREATE ALIAS QTEMP.IMOQ_BNDR FOR QTEMP.IMOQBND ('
       + %trim(obj) + ')';
  exec sql execute immediate :stmt;
  if sqlcode < 0;
    code = sqlcode;
    exec sql get diagnostics condition 1 :t = message_text;
    msg = 'Cannot read binder source QTEMP/IMOQBND(' + %trim(obj)
        + '): SQLCODE ' + %char(code) + ' ' + t;
    return *off;
  endif;
  ...
  exec sql declare cBnd2 cursor for
    select srcdta from qtemp.imoq_bndr order by srcseq;
  exec sql open cBnd2;
  if sqlcode < 0;
    code = sqlcode;
    exec sql get diagnostics condition 1 :t = message_text;
